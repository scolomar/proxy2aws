...
file=aws2cloud.conf;
command=" sudo kubectl delete configmap $file --from-file $folder/$file --kubeconfig $kubeconfig ";
targets=" InstanceManager1 " ;
for target in $targets ; do
...
command=" sudo kubectl delete secret generic $file --from-file $folder/$file --kubeconfig $kubeconfig ";
targets=" InstanceManager1 " ;
for target in $targets ; do
  send_list_command "$command" "$target" "$stack" ;
done ;
...
for app in $apps;
do
  command=" sudo kubectl delete --filename $folder/$app.yaml --kubeconfig $kubeconfig ";
  targets=" InstanceManager1 ";
